/**
 * GameRenderer.ts
 * Рендеринг игры с использованием PixiJS
 */
...
export class GameRenderer {
  ...
  private links: PIXI.Graphics[] = []; // Исправлено: массив графических объектов
  ...
  render(): void {
    ...
    this.renderObjects();
    this.renderLinks();
    if (this.gameStateManager.battleActive) {
      this.renderBattle();
    }
  }
  ...
  private renderPlayers(): void {
    ...
    // Render each player
    this.gameStateManager.allPlayers.forEach((player, playerId) => {
  ...
  private renderObjects(): void {
    ...
    // Render each object
    this.gameStateManager.allObjects.forEach((obj, objectId) => {
  ...
  private updateLinkPosition(link: PIXI.Graphics, cursor1: PIXI.Sprite, cursor2: PIXI.Sprite): void {
    if (!(link as any).userData) return; // Skip if userData is not set

    const player1Id = (link as any).userData.player1;
    const player2Id = (link as any).userData.player2;
  ...
  private renderBattle(): void {
    if (!this.app || !this.gameStateManager.battleActive) return;
  ...
  private updateBattleVisualization(): void {
    if (!this.battleVisualization || !this.gameStateManager.battleGrid) return;
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const idx = y * width + x;
        const cellState = this.gameStateManager.battleGrid[idx];
